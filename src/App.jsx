...
function App() {
  ...
  */
  ...
  const addToCart = (product) => {
    ...
    const productExit = CartItem.find((item) => item.idMeal === product.idMeal)
    ...
    if (productExit) {
      setCartItem(CartItem.map((item) => (item.idMeal === product.idMeal ? { ...productExit, qty: productExit.qty + 1 } : item)))
    } else {
  ...
  const decreaseQty = (product) => {
    ...
    if (productExit.qty === 1) {
      setCartItem(CartItem.filter((item) => item.idMeal !== product.idMeal))
    } else {
